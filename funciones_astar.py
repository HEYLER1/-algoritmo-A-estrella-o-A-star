...
def puede_moverse_diagonal(fila_actual, col_actual, fila_destino, col_destino, obstaculos):
    """
    Verifica si se puede mover en diagonal respetando las restricciones.
    
    RESTRICCIONES DIAGONALES:
    Para moverse en diagonal, AMBAS celdas adyacentes deben estar libres.
    
    Ejemplo: Para ir de A a B (diagonal):
        [ ][ ][B]
        [ ][A][ ]
        
    Se debe verificar que las celdas marcadas con ✓ estén libres:
        [ ][✓][B]
        [✓][A][ ]
    
    Args:
        fila_actual: Fila de la celda actual
        col_actual: Columna de la celda actual
        fila_destino: Fila de la celda destino
        col_destino: Columna de la celda destino
        obstaculos: Set de tuplas con posiciones de obstáculos
    
    Returns:
        bool: True si se puede mover, False si está bloqueado
    """
    # Verificar que el destino no sea un obstáculo
    if (fila_destino, col_destino) in obstaculos:
        return False
    
    # Obtener las dos celdas adyacentes que deben estar libres
    celda_horizontal = (fila_actual, col_destino)  # Misma fila, columna destino
    celda_vertical = (fila_destino, col_actual)     # Fila destino, misma columna
    
    # Ambas celdas adyacentes deben estar libres
    if celda_horizontal in obstaculos:
        return False
    
    if celda_vertical in obstaculos:
        return False
...
def obtener_vecinos(nodo, filas, columnas, obstaculos, permitir_diagonal=False):
    """
    Obtiene los vecinos válidos de un nodo respetando restricciones.
    ...
    Args:
        ...
        obstaculos: Set de tuplas con posiciones de obstáculos
        permitir_diagonal: Si True, permite movimientos diagonales con restricciones
    ...
    # Estos siempre están disponibles si no hay obstáculos
    ...
    for df, dc, direccion in movimientos_basicos:
        nueva_fila = fila + df
        nueva_col = col + dc
    ...
    # Solo si están permitidos y respetan las restricciones
    ...
    if permitir_diagonal:
        ...
        for df, dc, direccion in movimientos_diagonales:
            nueva_fila = fila + df
            nueva_col = col + dc
            ...
            # VERIFICAR RESTRICCIONES DIAGONALES
            # Debe poder moverse respetando las celdas adyacentes
            if puede_moverse_diagonal(fila, col, nueva_fila, nueva_col, obstaculos):
                vecinos.append((nueva_fila, nueva_col))
